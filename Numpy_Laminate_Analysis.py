...
__all__ = ["chop", "T_Matrix", "R_Matrix", "Poissons_Ratio",
           "get_Elastic_Constants", "Material", "Iso_Material",
...
def T_Matrix(angle):
    ...
    return chop(np.matrix([[c ** 2, s ** 2, 2 * s * c],
                           [s ** 2, c ** 2, -2 * s * c],
                           [-s * c, s * c, c ** 2 - s ** 2]]))
...
def Poissons_Ratio(vij, Ei, Ej):
    """
    Calculates other poisson's ratio vji = vij*(Ej/Ei)
    """
    return vij * Ej / Ei
...
class Ortho_Material(Material):
    def __init__(self, E1, E2, G1, G2, v12=None, v21=None):
        ...
        """
        assert v12 is not None or v21 is not None, "Must supply v12 or v21."
...
class Ply(object):
    def __init__(self, E11, E22, G12, v12, v21):
        """
        Create Ply instance with given elastic constants
        Parameters
        ----------
        E11 : 'Int' or 'Float'
            Axial Young's Modulus.
    ...
    def get_S(self, theta=0., strain_type="Engineering"):
        ...
        compliance = chop(np.matrix([[1 / self.E11, -self.v21 / self.E22, 0],
                          [-self.v12 / self.E11, 1 / self.E22, 0],
                          [0, 0, 1 / (strain_multiplier * self.G12)]]))

        if theta == 0.:
            return compliance.astype(dtype='float')
        else:
            T = T_Matrix(theta)
            if strain_type.lower().startswith('e'):
                R = R_Matrix()
                ...
                TI = T.I
            return chop(TI * compliance * T).astype(dtype='float')
    ...
    def get_Q(self, theta=0., strain_type="Engineering"):
        ...
        return chop(self.get_S(theta, strain_type).I)
...
class GSCS_Ply(Ply):
    def __init__(self, fiber, matrix, Vf):
        ...
        # Fiber Properties
        (Eaf, Etf, Gaf, Gtf, vaf, vtf, cf) = (fiber.E1, fiber.E2, fiber.G1,
                                              fiber.G2, fiber.v12, fiber.v21,
                                              Vf)
        kf = (Eaf * Etf / (2 * Eaf - 4 * Etf * vaf ** 2 - 2 * Eaf * vtf))
        etaf = (3 - 4 * 1 / 2 * (1 - Gtf / kf))

        # Matrix Properties
        (Eam, Etm, Gam, Gtm, vam, vtm, cm) = (matrix.E1, matrix.E2, matrix.G1,
                                              matrix.G2, matrix.v12,
                                              matrix.v21, 1 - Vf)
        km = (Eam * Etm / (2 * Eam - 4 * Etm * vam ** 2 - 2 * Eam * vtm))
        # mm = (1 + 4 * km * vam ** 2 / Eam)
        etam = (3 - 4 * 1 / 2 * (1 - Gtm / km))

        # Axial Ply Properties (Hashin)
        Eac = (Eam * cm + Eaf * cf + 4 * (vaf - vam) ** 2 * cm * cf /
               (cm / kf + cf / km + 1 / Gtm))
        vac = (vam * cm + vaf * cf + (vaf - vam) * (1 / km - 1 / kf) *
               cm * cf / (cm / kf + cf / km + 1 / Gtm))
        Gac = (Gam * (Gam * cm + Gaf * (1 + cf)) / (Gam * (1 + cf) + Gaf * cm))
        kc = ((km * (kf + Gtm) * cm + kf * (km + Gtm) * cf) /
              ((kf + Gtm) * cm + (km + Gtm) * cf))
        Gtr = (Gtf / Gtm)
        mc = (1 + 4 * kc * vac ** 2 / Eac)
        ...
        Bchr = (-3 * cf * cm ** 2 * (Gtr - 1) * (Gtr + etaf) +
                1 / 2 * (etam * Gtr + (Gtr - 1) * cf + 1) *
                ((etam - 1) * (Gtr + etaf) - 2 * (Gtr * etam - etaf) *
                cf ** 3) + cf / 2 * (etam + 1) * (Gtr - 1) *
                (Gtr + etaf + (Gtr * etam - etaf) * cf ** 3))
        Cchr = (3 * cf * cm ** 2 * (Gtr - 1) * (Gtr + etaf) +
                (etam * Gtr + (Gtr - 1) * cf + 1) *
                (Gtr + etaf + (Gtr * etam - etaf) * cf ** 3))
...
class Laminate(object):
    def __init__(self, Plies, Layup, t_Plies=None, strain_type="Engineering"):
        ...
        """
        if t_Plies is None:
            assert len(Plies) == len(Layup), "# of Plies must = # of angles."
        else:
...
number of Plies and angles."
        ...
        z = np.hstack((np.zeros(1), np.cumsum(t_Plies))) - t / 2
        ...
        self.A = chop(np.asmatrix(A))
        self.B = chop(np.asmatrix(B))
        self.D = chop(np.asmatrix(D))

        (E11,
         E22,
         G12,
         v12,
         v21) = get_Elastic_Constants(chop(self.A.I * self.t),
